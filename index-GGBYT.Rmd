...
1. Raspberry Pi SensorGnome Cheatsheet {#rpicheatsheet}
...
2. BeagleBone SensorGnome Cheatsheet {#bbcheatsheet}
...
```{r echo=F, fig.cap='The button on the left is used to activate the Wifi hotspot'}
...
2.	The WiFi hotspot should soon appear in the list of available WiFi networks to connect to. The name of the WiFi Network will be the same as the serial number of the RPi SG.  

    a)	It can take up to a minute for the WiFi network to appear  
  
    b)	The LED light in the button is usually bright enough to view in sunlight, but occasionally it is very faint and difficult to see  
  
    c)	The timing of the double press can be finicky and difficult to activate. If it doesn’t work, try it again.  
...
4.	Open a web browser (Firefox or Chrome, or your smartphone browser)  
...
1. **Install BeagleBones drivers**. Direct link to x64 Windows Driver [here](https://beagleboard.org/static/Drivers/Windows/BONE_D64.exe). More information here and other versions here, about halfway down the page: http://beagleboard.org/getting-started  
    ...
    + Note: After June 2016, drivers are apparently no longer needed (according to https://archived.sensorgnome.org/Connecting_to_your_SensorGnome/Installing_drivers_for_the_SensorGnome_on_your_computer/) but older versions of the software may still be encountered.  
...
**The contents Web Interface is the same regardless of whether you have a BeagleBone or Raspberry Pi SensorGnome**
...
1) Connect to the SensorGnome using the process described above
...
**Raspberry Pi SG**  
  ...
  Note: when connnecting via WiFi Hotpsot, you may also use the host address: sftp://192.168.7.2 
...
```{r echo=F, fig.cap='Enter the host address and credentials in the top left.', fig.align='center'}
...
# Other methods of transferring data
